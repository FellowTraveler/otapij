...
public class Utility {
    ...
    public static boolean insureHaveAllBoxReceipts(String serverID, String nymID, String accountID, int nBoxType) {
        ...
        if ((null == ledger)
                || (ledger.length() < 10)
                || (0 == otapi.OT_API_VerifySignature(nymID, ledger))) {
    ...
    public static boolean getTransactionNumbers(String serverID, String nymID) {
        ...
        for (int i = 0; i < Configuration.getNbrTransactionCount(); i++) {
            ...
            if ((serverResponse == null) || (serverResponse.length() < 10)) {
                System.out.println("Utility.getTransactionNumbers(): server reply empty. Perhaps the receive_fail_ms in client.cfg needs to be set to a higher value?");
                bSuccess = false;
            }
            // -----------------------------------------------------------
            // REPLY: FAILURE
